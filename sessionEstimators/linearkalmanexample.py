...
def discrete_aircraft_model_3d(dt):
    """Generate discrete-time model matrices for 3D constant-velocity tracking."""
    A = np.eye(6)
...
### Animated version
def animate_3d_trajectory_with_history(x_true, y_meas, x_est):
    """
    Animate 3D aircraft trajectory in Plotly with history (growing traces).

    Args:
        x_true: True states (6, N)
        y_meas: Radar measurements (3, N)
        x_est: Estimated states (6, N)
    """
    N = x_true.shape[1]
    ...
    fig = go.Figure(
        layout=go.Layout(
            title="3D Aircraft Tracking with Kalman Filter (History Animation)",
            scene=dict(
                xaxis_title='X (m)',
                yaxis_title='Y (m)',
                zaxis_title='Z (m)'
    ...
    fig.add_trace(go.Scatter3d(mode='lines+markers', name='True Trajectory',
                               x=[], y=[], z=[], line=dict(color='blue')))
    fig.add_trace(go.Scatter3d(mode='markers', name='Radar Measurements',
                               x=[], y=[], z=[], marker=dict(size=3, color='red', opacity=0.6)))
    fig.add_trace(go.Scatter3d(mode='lines+markers', name='Kalman Estimate',
                               x=[], y=[], z=[], line=dict(color='green', dash='dash')))

    # Create frames with cumulative history
    frames = []
    for k in range(N):
        frame = go.Frame(
            data=[
                go.Scatter3d(x=x_true[0, :k+1], y=x_true[2, :k+1], z=x_true[4, :k+1]),       # True
                go.Scatter3d(x=y_meas[0, :k+1], y=y_meas[1, :k+1], z=y_meas[2, :k+1]),       # Measured
                go.Scatter3d(x=x_est[0, :k+1], y=x_est[2, :k+1], z=x_est[4, :k+1])           # Estimated
            ],
            name=str(k)
        )
        frames.append(frame)
...
def main():
    ...
    #Animated version
    animate_3d_trajectory_with_history(x_true, y_meas, x_est)
